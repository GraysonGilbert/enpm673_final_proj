...
class TB3ArUcoFollower(Node):
    ...
    def __init__(self):
        ...
        self.camera_offset_x = -0.0476  # meters. From SDF file (camera is behind base_footprint)
        self.camera_offset_y = 0.0      # meters. From SDF file
        self.robot_radius = 0.1325      # meters. From SDF file
                ...
                if len(filtered_ids) > 0:
                    ...
                    for i in range(len(filtered_ids)):
                        ...
                        if self.camera_matrix is not None and self.dist_coeffs is not None:
                            ...
                            # --- Compensate for camera offset in robot frame (only for tb3 simulation, comment-out for tb4) ---
                            # Marker position in robot frame = camera offset + marker position relative to camera
                            marker_in_robot_x = self.camera_offset_x + robot_x
                            marker_in_robot_y = self.camera_offset_y + robot_y
                        ...
                        new_marker_positions.append({
                            ...
                            'distance': math.hypot(robot_x, robot_y),
                            'last_seen': time.time()
                        })
    ...
    def follow_aruco_control(self, dt):
        ...
        if self.marker_queue:
            ...
            dist_err = math.hypot(dx, dy) - self.robot_radius
